...
#define EXAMPLE_MQTT_PSWD "K64"

/*! @brief MQTT server port number. */
#define EXAMPLE_MQTT_SERVER_PORT 14403
//#endif
...
static volatile bool g_ButtonPress = false;
static volatile bool g_Button2Press = false;
...
void BOARD_SW_IRQ_HANDLER(void)   //Push button ISR
{
...
{
    static const char *topics[] = {/*"/sprinkler", */"/OpenDoor"};
    int qos[]                   = {1/*, 2*/};
    err_t err;
    int i;
...
static void publish_backyard()
{
    static const char *topic   = "/backyard";
    static const char *message   = "Violation in the Backyard";
    PRINTF("Going to publish to the topic \"%s\"...\r\n", topic);
    mqtt_publish(mqtt_client, topic, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}

static void publish_frontyard()
{
    static const char *topic   = "/frontyard";
    static const char *message   = "Violation in the Frontyard";
    PRINTF("Going to publish to the topic \"%s\"...\r\n", topic);
    mqtt_publish(mqtt_client, topic, message, strlen(message), 1, 0, mqtt_message_published_cb, (void *)topic);
}
